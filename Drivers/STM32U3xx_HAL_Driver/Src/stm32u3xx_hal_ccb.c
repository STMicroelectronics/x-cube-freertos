...
#define HAL_CCB_TIMEOUT_DEFAULT_VALUE      0xFFFFFU         /* CCB Timeout.*/
...
/** @defgroup CCB_Operations CCB Operations
  * @{
  */
#define HAL_CCB_ECDSA_SIGN_CPU_BLOB_CREATION     (0x000000C0U)  /*!< ECDSA signature blob creation with priv key from the CPU */
#define HAL_CCB_ECDSA_SIGN_RNG_BLOB_CREATION     (0x000000C2U)  /*!< ECDSA signature blob creation with priv key from the RNG */
#define HAL_CCB_ECDSA_SIGN_BLOB_USE              (0x000000C3U)  /*!< ECDSA signature blob use for ECDSA signature */
#define HAL_CCB_ECC_SCALAR_MUL_CPU_BLOB_CREATION (0x00000080U)  /*!< ECC scalar multiplication blob creation with priv key from the CPU- */
#define HAL_CCB_ECC_SCALAR_MUL_RNG_BLOB_CREATION (0x00000082U)  /*!< ECC scalar multiplication blob creation with priv key from the RNG */
#define HAL_CCB_ECC_SCALAR_MUL_BLOB_USE          (0x00000081U)  /*!< ECC Scalar multiplication blob use, or ECDSA signature blob use for public key computation */
#define HAL_CCB_MODULAR_EXP_CPU_BLOB_CREATION    (0x00000044U)  /*!< Modular exponentiation blob creation with priv key from the CPU */
#define HAL_CCB_MODULAR_EXP_BLOB_USE             (0x00000045U)  /*!< Modular exponentiation blob use */
...
/** @defgroup CCB_PKA_Mode CCB PKA mode
  * @{
  */
#define HAL_CCB_PKA_MODE_MODULAR_EXP_PROTECT     (0x00000003U)  /*!< PKA Modular exponentiation */
#define HAL_CCB_PKA_ECC_MUL_MODE                 (0x00000020U)  /*!< PKA ECC scalar multiplication */
#define HAL_CCB_PKA_ECDSA_SIGNATURE_MODE         (0x00000024U)  /*!< PKA ECDSA signature */
#define HAL_CCB_PKA_MODE_ECDSA_VERIFICATION      (0x00000026U)  /*!< PKA ECDSA verification */
#define HAL_CCB_PKA_ERROR_OPERATION_NONE         (0x0000D60DU)  /*!< No PKA Hardware operation error */
...
#define CCB_PKA_RAM_SIZE             0x0536UL /*!< CCB PKA Ram Size                                          */
...
#define HAL_CCB_GET_PKA_FLAG(__INSTANCE__, __FLAG__)              ((((PKA->SR)\
                                                                     & (__FLAG__)) == (__FLAG__)) ? SET : RESET)

#define HAL_CCB_CLEAR_PKA_FLAG(__INSTANCE__, __FLAG__)            WRITE_REG(PKA->CLRFR, (__FLAG__))
...
static HAL_StatusTypeDef WrappingKeyConfiguration(CCB_HandleTypeDef *hccb, uint32_t Operation,
                                                  const CCB_WrappingKeyTypeDef *pWrappingKey);
...
static HAL_StatusTypeDef CCB_BlobUse_InitialPhase(CCB_HandleTypeDef *hccb, const uint32_t *IV, uint32_t *Tag);
static HAL_StatusTypeDef CCB_BlobCreation_FinalPhase(CCB_HandleTypeDef *hccb, uint32_t Operation, uint32_t *Tag,
                                                     uint32_t sizeparam);
...
static HAL_StatusTypeDef CCB_ECC_ComputeScalarMul(CCB_HandleTypeDef *hccb, CCB_ECCMulCurveParamTypeDef *pCurveParam,
                                                  const CCB_WrappingKeyTypeDef *pWrappingKey,
                                                  const CCB_ECCMulKeyBlobTypeDef *pWrappedPrivateKeyBlob,
...
static HAL_StatusTypeDef CCB_RSA_ComputeModularExp(CCB_HandleTypeDef *hccb, CCB_RSAParamTypeDef *pParam,
                                                   ...
                                                   uint8_t *pModularExp, const uint32_t *pReferenceModularExp,
                                                   uint8_t VerifOperation);
static HAL_StatusTypeDef PKA_ECDSASign(CCB_ECDSACurveParamTypeDef *pCurveParam,
                                       uint8_t *pClearPrivateKey, uint8_t *pInteger, uint8_t *pHash,
                                       CCB_ECDSASignTypeDef *pSignature);

static HAL_StatusTypeDef PKA_ECC_ComputeScalarMul(CCB_ECCMulCurveParamTypeDef *pCurveParam, uint8_t *pClearPrivateKey,
                                                  uint32_t PublicKey[2][20]);
...
static HAL_StatusTypeDef PKA_ECDSAVerif(CCB_ECDSACurveParamTypeDef *pCurveParam,
                                        ...
                                        CCB_ECDSASignTypeDef *pSignature);
static uint32_t PKA_ECDSAVerif_IsValidSignature(void);
...
{
  __IO uint8_t count;
  uint32_t count_rundom;
  uint8_t RandomNbr;
  uint8_t ECDSA_Hash[80U] = {0};
  uint8_t PKA_rSign[80U] = {0};
  uint8_t PKA_sSign[80U] = {0};
  uint8_t CCB_PointX[80U] = {0};
  uint8_t CCB_PointY[80U] = {0};
  uint8_t ECDSA_Integer[80U] = {0};

  CCB_ECDSASignTypeDef Signature = {PKA_rSign, PKA_sSign};
  CCB_ECCMulPointTypeDef PublicKeyOut = {(uint8_t *)CCB_PointX, (uint8_t *)CCB_PointY};
  ...
  for (count_rundom = 0U; count_rundom < (pCurveParam->primeOrderSizeByte); count_rundom++)
  {
    ECDSA_Hash[count_rundom] = (uint8_t)((RNG->DR) & 0x000000FFU);
  ...
  for (count_rundom = 0U; count_rundom < (pCurveParam->primeOrderSizeByte); count_rundom++)
  {
    ECDSA_Integer[count_rundom] = (uint8_t)((RNG->DR) & 0x000000FFU);
  ...
  if (hccb->State == HAL_CCB_STATE_READY)
  {
    ...
    if (PKA_ECDSASign(pCurveParam, pClearPrivateKey, (uint8_t *)ECDSA_Integer, (uint8_t *)ECDSA_Hash,
                      &Signature) != HAL_OK)
    {
    ...
    RandomNbr = (uint8_t)(RNG->DR & 0xFFU);
    count = 0;
    while (count < RandomNbr)
    {
      count++;
    }

    /* Signature blob creation */
    if (CCB_ECDSA_SignBlobCreation(hccb, pCurveParam, pClearPrivateKey, pWrappingKey, pWrappedPrivateKeyBlob,
                                   HAL_CCB_ECDSA_SIGN_CPU_BLOB_CREATION) != HAL_OK)
    {
    ...
    if (CCB_ECC_ComputeScalarMul(hccb, pCurveParam, pWrappingKey, pWrappedPrivateKeyBlob, NULL,
                                 &PublicKeyOut, NULL, CCB_COMPUTE_PUBLIC_KEY) != HAL_OK)
    {
      /* Set state, error code and return error */
      hccb->State = HAL_CCB_STATE_ERROR;
      return HAL_ERROR;
    ...
    if (PKA_ECDSAVerif(pCurveParam, &PublicKeyOut, ECDSA_Hash, &Signature) != HAL_OK)
    {
      return HAL_ERROR;
    }

    /* CCB ECDSA verif is valid signature */
    if (PKA_ECDSAVerif_IsValidSignature() != 1U)
    {
      return HAL_ERROR;
    }
                                                       ...
                                                       CCB_ECDSAKeyBlobTypeDef *pWrappedPrivateKeyBlob)
{
  ...
  if (hccb->State == HAL_CCB_STATE_READY)
  {
    ...
    /* Signature blob creation */
    if (CCB_ECDSA_SignBlobCreation(hccb, pCurveParam, NULL, pWrappingKey, pWrappedPrivateKeyBlob,
                                   HAL_CCB_ECDSA_SIGN_RNG_BLOB_CREATION) != HAL_OK)
    {
      /* Set state, error code and return error */
      hccb->State = HAL_CCB_STATE_ERROR;
      return HAL_ERROR;
...

{
  ...
  if (hccb->State == HAL_CCB_STATE_READY)
  {
    ...
    MODIFY_REG(CCB->CR, CCB_CR_CCOP, HAL_CCB_ECDSA_SIGN_BLOB_USE);
    ...
    /* Initialize PKA */
    if (Protect_PKA_Init(hccb, HAL_CCB_ECDSA_SIGN_BLOB_USE) != HAL_OK)
    {
      /* Set state and return error */
      hccb->State = HAL_CCB_STATE_ERROR;
      return HAL_ERROR;
    ...
    if (WrappingKeyConfiguration(hccb, HAL_CCB_ECDSA_SIGN_BLOB_USE, pWrappingKey) != HAL_OK)
    {
      /* Set state, error code and return error */
      return HAL_ERROR;
    }
    ...
    for (offset = 0UL; offset < cipherkey_size; offset++)
    {
      ...
      if ((offset % 4UL) == 0x3UL)
      {
        ...
        {
          PKA->RAM[PKA_ECDSA_SIGN_IN_PRIVATE_KEY_D + (countBlock + count)] = CCB_MAGIC_VALUE;
        }
        countBlock += 4UL;
      }
    }
    ...
    if (CCB_BlobUse_FinalPhase(hccb, HAL_CCB_ECDSA_SIGN_BLOB_USE, pCurveParam->modulusSizeByte) != HAL_OK)
    {
      /* return error */
      return HAL_ERROR;
    }
    ...
    if ((PKA->RAM[PKA_ECDSA_SIGN_OUT_ERROR]) !=  HAL_CCB_PKA_ERROR_OPERATION_NONE)
    {
      /* Set state and return error */
      hccb->State = HAL_CCB_STATE_ERROR;
      return HAL_ERROR;
                                                 ...
                                                 CCB_ECCMulPointTypeDef *pPublicKey)
{
  ...
  {
    if (CCB_ECC_ComputeScalarMul(hccb, pCurveParam, pWrappingKey, pWrappedPrivateKeyBlob, NULL,
                                 pPublicKey, NULL, CCB_COMPUTE_PUBLIC_KEY) != HAL_OK)
    {
      return HAL_ERROR;
    }
  }
...
{
  uint8_t RandomNbr;
  uint32_t PublicKey[2][20] = {{0UL}, {0UL}};
  __IO uint8_t count;
  ...
  if (hccb->State == HAL_CCB_STATE_READY)
  {
    ...
    if (PKA_ECC_ComputeScalarMul(pCurveParam, pClearPrivateKey, PublicKey) != HAL_OK)
    {
      if (PKA_RAM_Erase() != HAL_OK)
      {
        hccb->ErrorCode |= HAL_CCB_ERROR_PKARAM_ERASE;
    ...
    if (CCB_ECC_ScalarMulBlobCreation(hccb, pCurveParam, pClearPrivateKey, pWrappingKey,
                                      pWrappedPrivateKeyBlob,
                                      HAL_CCB_ECC_SCALAR_MUL_CPU_BLOB_CREATION) != HAL_OK)
    {
      return HAL_ERROR;
    }
    ...
    if (CCB_ECC_ComputeScalarMul(hccb, pCurveParam, pWrappingKey, pWrappedPrivateKeyBlob, NULL,
                                 NULL, &PublicKey[0][0], CCB_VERIF_OPERATION_ENABLED) != HAL_OK)
    {
      return HAL_ERROR;
    }
                                                     ...
                                                     CCB_ECCMulKeyBlobTypeDef *pWrappedPrivateKeyBlob)
{
  ...
  if (hccb->State == HAL_CCB_STATE_READY)
  {
    ...
    if (CCB_ECC_ScalarMulBlobCreation(hccb, pCurveParam, NULL, pWrappingKey,
                                      pWrappedPrivateKeyBlob,
                                      HAL_CCB_ECC_SCALAR_MUL_RNG_BLOB_CREATION) != HAL_OK)
    {
      return HAL_ERROR;
    }
  }
                                               ...
                                               CCB_ECCMulPointTypeDef *pOutputPoint)
{
  if (hccb->State == HAL_CCB_STATE_READY)
  {
    if (CCB_ECC_ComputeScalarMul(hccb, pCurveParam, pWrappingKey, pWrappedPrivateKeyBlob, pInputPoint,
                                 pOutputPoint, NULL, CCB_VERIF_OPERATION_DISABLED) != HAL_OK)
    {
      return HAL_ERROR;
    }
  }
...
HAL_StatusTypeDef HAL_CCB_RSA_WrapPrivateKey(CCB_HandleTypeDef *hccb, CCB_RSAParamTypeDef *pParam,
                                             ...
                                             CCB_RSAKeyBlobTypeDef *pWrappedPrivateKeyBlob)
{
  uint32_t Ref_modular_exp[130U] = {0};
  uint8_t  RSA_expBase[520U] = {0};
  uint8_t RandomNbr;
  __IO uint8_t count;
  ...
  if (hccb->State == HAL_CCB_STATE_READY)
  {
    ...
    for (uint32_t offset = 0U; offset < pParam->modulusSizeByte; offset++)
    {
      ...
      RandomNbr = (uint8_t)((RNG->DR) & 0xFFU);
      RSA_expBase[offset] = (uint8_t)RandomNbr;
    ...
    if (PKA_RSA_ComputeModularExp(pParam, pRSAClearPrivateKey, (uint8_t *)RSA_expBase, Ref_modular_exp) != HAL_OK)
    {
      if (PKA_RAM_Erase() != HAL_OK)
      {
        hccb->ErrorCode |= HAL_CCB_ERROR_PKARAM_ERASE;
    ...
    if (CCB_RSA_ExpBlobCreation(hccb, pParam, pRSAClearPrivateKey, pWrappingKey, pWrappedPrivateKeyBlob) != HAL_OK)
    {
      return HAL_ERROR;
    }
    if (CCB_RSA_ComputeModularExp(hccb, pParam, pWrappingKey, pWrappedPrivateKeyBlob, (uint8_t *)RSA_expBase,
                                  NULL, Ref_modular_exp, CCB_VERIF_OPERATION_ENABLED) != HAL_OK)
    {
      return HAL_ERROR;
    }
  }
                                                ...
                                                uint8_t *pModularExp)
{
  if (hccb->State == HAL_CCB_STATE_READY)
  {
    if (CCB_RSA_ComputeModularExp(hccb, pParam, pWrappingKey, pWrappedPrivateKeyBlob, pOperand, pModularExp,
                                  NULL, CCB_VERIF_OPERATION_DISABLED) != HAL_OK)
    {
      return HAL_ERROR;
    }
  }
                                               ...
                                               uint32_t Timeout)
{
  ...
  if (Status == SET)
  {
    while (HAL_IS_BIT_CLR(SAES->ISR, flag))
    {
      if (((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U))
      {
        SAES->CR &= ~AES_CR_EN;
  ...
  {
    while (HAL_IS_BIT_SET(SAES->ISR, flag))
    {
      if (((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U))
      {
        SAES->CR &= ~AES_CR_EN;
...
static HAL_StatusTypeDef Protect_PKA_Init(CCB_HandleTypeDef *hccb, uint32_t Operation)
{
  ...
  if ((Operation == HAL_CCB_ECDSA_SIGN_CPU_BLOB_CREATION)
      || (Operation == HAL_CCB_ECDSA_SIGN_RNG_BLOB_CREATION)
      || (Operation == HAL_CCB_ECDSA_SIGN_BLOB_USE))
  {
    MODIFY_REG(PKA->CR, PKA_CR_MODE | PKA_CR_PROCENDIE | PKA_CR_RAMERRIE | PKA_CR_ADDRERRIE
               | PKA_CR_OPERRIE, HAL_CCB_PKA_ECDSA_SIGNATURE_MODE << PKA_CR_MODE_Pos);
  }

  else if ((Operation == HAL_CCB_ECC_SCALAR_MUL_CPU_BLOB_CREATION)
           || (Operation == HAL_CCB_ECC_SCALAR_MUL_RNG_BLOB_CREATION)
           || (Operation == HAL_CCB_ECC_SCALAR_MUL_BLOB_USE))
  {
    MODIFY_REG(PKA->CR, PKA_CR_MODE | PKA_CR_PROCENDIE | PKA_CR_RAMERRIE | PKA_CR_ADDRERRIE
               | PKA_CR_OPERRIE, HAL_CCB_PKA_ECC_MUL_MODE << PKA_CR_MODE_Pos);
  }

  else if ((Operation == HAL_CCB_MODULAR_EXP_CPU_BLOB_CREATION)
           || (Operation == HAL_CCB_MODULAR_EXP_BLOB_USE))
  {
    MODIFY_REG(PKA->CR, PKA_CR_MODE | PKA_CR_PROCENDIE | PKA_CR_RAMERRIE | PKA_CR_ADDRERRIE
               | PKA_CR_OPERRIE, HAL_CCB_PKA_MODE_MODULAR_EXP_PROTECT << PKA_CR_MODE_Pos);
  }
  else
  {
    /* do nothing */
                                                  ...
                                                  const CCB_WrappingKeyTypeDef *pWrappingKey)
{
  ...
  switch (pWrappingKey->WrappingKeyType)
  {
    case HAL_CCB_USER_KEY_WRAPPED:
      ...
      if ((Operation == HAL_CCB_ECDSA_SIGN_CPU_BLOB_CREATION) || (Operation == HAL_CCB_ECDSA_SIGN_RNG_BLOB_CREATION)
          || (Operation == HAL_CCB_ECC_SCALAR_MUL_CPU_BLOB_CREATION)
          || (Operation == HAL_CCB_ECC_SCALAR_MUL_RNG_BLOB_CREATION)
          || (Operation == HAL_CCB_MODULAR_EXP_CPU_BLOB_CREATION))
      {
      ...
      else if ((Operation == HAL_CCB_ECDSA_SIGN_BLOB_USE) || (Operation == HAL_CCB_ECC_SCALAR_MUL_BLOB_USE)
               || (Operation == HAL_CCB_MODULAR_EXP_BLOB_USE))
      {
                                                ...
                                                uint32_t Timeout)
{
  ...
  /* Set the phase */
  while (SAESInCount < 8UL) /* symmetric key size is always 256 */
  {
    /* Write four times to input the key to encrypt */
    SAES->DINR  = pWrappingKey->pUserWrappedWrappingKey[SAESInCount];
    SAESInCount++;
    SAES->DINR  = pWrappingKey->pUserWrappedWrappingKey[SAESInCount];
    SAESInCount++;
    SAES->DINR  = pWrappingKey->pUserWrappedWrappingKey[SAESInCount];
...
/**
  ...
  * @param  hccb CCB handle
  * @param  IV pointer to the initil vector
  * @retval HAL status
  */
static HAL_StatusTypeDef CCB_BlobCreation_InitialPhase(CCB_HandleTypeDef *hccb, uint32_t *IV)
{
...
static HAL_StatusTypeDef CCB_BlobUse_InitialPhase(CCB_HandleTypeDef *hccb, const uint32_t *IV, uint32_t *Tag)
{
  ...
  {
    WRITE_REG(hccb->Instance->REFTAGR[count], Tag[count]);
  }
                                                     ...
                                                     uint32_t sizeparam)
{
  ...
  if (Operation == HAL_CCB_MODULAR_EXP_CPU_BLOB_CREATION)
  {
    /* bitsize of exp length, modulus n length  and modulus (address and data)*/
    LastBlock[1] = ((64UL * 2UL) + (operand_size * 32UL)) * 2UL;

    /* ciphred data size*/
    LastBlock[3] = cipherkey_size * 32UL;
  }

  else if ((Operation == HAL_CCB_ECDSA_SIGN_CPU_BLOB_CREATION) || (Operation == HAL_CCB_ECDSA_SIGN_RNG_BLOB_CREATION))
  {
    /* bitsize of |a|, b, p, n, Gx and Gy --> sizeparam*8*6*2 (address and data); n length,
    plength and a sign --> 64*3*2 (address and data) */
    LastBlock[1] = (((operand_size * 32UL) * 6UL) + (3UL * 64UL)) * 2UL;
  ...
  else
  {
    ...
    LastBlock[1] = (((operand_size * 32UL) * 4UL) + (3UL * 64UL)) * 2UL;
...
static HAL_StatusTypeDef CCB_BlobUse_FinalPhase(CCB_HandleTypeDef *hccb, uint32_t Operation, uint32_t sizeparam)
{
  ...
  /* Preparing last Block */
  if (Operation == HAL_CCB_MODULAR_EXP_BLOB_USE)
  {
    /* bitsize of exp length, modulus n length  and modulus (address and data)*/
    LastBlock[1] = ((64UL * 2UL) + (operand_size * 32UL)) * 2UL;
  ...
  else if (Operation == HAL_CCB_ECDSA_SIGN_BLOB_USE)
  {
    /* bitsize of |a|, b, p, n, Gx and Gy --> sizeparam*8*6*2 (address and data); n length,
    plength and a sign --> 64*3*2 (address and data) */
    LastBlock[1] = (((operand_size * 32UL) * 6UL) + (3UL * 64UL)) * 2UL;
                                                    ...
                                                    uint8_t CCB_Operation)
{
  ...
  /* Initial Phase processing */
  if (CCB_BlobCreation_InitialPhase(hccb, pWrappedPrivateKeyBlob->pIV) != HAL_OK)
  {
    /* return error */
    return HAL_ERROR;
  }
  ...
  if (CCB_Operation == HAL_CCB_ECDSA_SIGN_CPU_BLOB_CREATION)
  {
  ...
  else if (CCB_Operation == HAL_CCB_ECDSA_SIGN_RNG_BLOB_CREATION)
  {
  ...
  for (offset = 0; offset < cipherkey_size; offset++)
  {
    ...
    if ((offset % 4UL) == 3UL)
    {
      ...
      for (count = 0U; count < CCB_BLOCK_SIZE; count++)
      {
        pWrappedPrivateKeyBlob->pWrappedKey[cipherkey_size - (countBlock + count + 1UL)]
          = READ_REG(SAES->DOUTR);
      }
      countBlock += 4UL;
    }
  ...
  /* Final phase processing */
  if (CCB_BlobCreation_FinalPhase(hccb, CCB_Operation, pWrappedPrivateKeyBlob->pTag,
                                       ...
                                       CCB_ECDSASignTypeDef *pSignature)
{
  ...
  MODIFY_REG(PKA->CR, PKA_CR_MODE | PKA_CR_PROCENDIE | PKA_CR_RAMERRIE | PKA_CR_ADDRERRIE | PKA_CR_OPERRIE,
             HAL_CCB_PKA_ECDSA_SIGNATURE_MODE << PKA_CR_MODE_Pos);
...
static HAL_StatusTypeDef PKA_ECC_ComputeScalarMul(CCB_ECCMulCurveParamTypeDef *pCurveParam,
                                                  uint8_t *pClearPrivateKey, uint32_t PublicKey[2][20])
{
  uint32_t tickstart;
  uint32_t Timeout = HAL_CCB_TIMEOUT_DEFAULT_VALUE;
  ...
  /* Set the mode and deactivate the interrupts */
  MODIFY_REG(PKA->CR, PKA_CR_MODE | PKA_CR_PROCENDIE | PKA_CR_RAMERRIE | PKA_CR_ADDRERRIE | PKA_CR_OPERRIE,
             HAL_CCB_PKA_ECC_MUL_MODE << PKA_CR_MODE_Pos);
                                                   ...
                                                   uint8_t *pOp1, uint32_t *pReferenceModularExp)
{
  ...
  RAM_PARAM_END(PKA->RAM, PKA_MODULAR_EXP_PROTECT_IN_EXPONENT_BASE + (pParam->modulusSizeByte / 4UL));
  ...
  RAM_PARAM_END(PKA->RAM, PKA_MODULAR_EXP_PROTECT_IN_EXPONENT + (pParam->expSizeByte / 4UL));
  ...
  RAM_PARAM_END(PKA->RAM, PKA_MODULAR_EXP_PROTECT_IN_MODULUS + (pParam->modulusSizeByte / 4UL));
  ...
  RAM_PARAM_END(PKA->RAM, PKA_MODULAR_EXP_PROTECT_IN_PHI + (pParam->modulusSizeByte / 4UL));

  /* Set the mode and deactivate the interrupts */
  MODIFY_REG(PKA->CR, PKA_CR_MODE | PKA_CR_PROCENDIE | PKA_CR_RAMERRIE | PKA_CR_ADDRERRIE | PKA_CR_OPERRIE,
             HAL_CCB_PKA_MODE_MODULAR_EXP_PROTECT << PKA_CR_MODE_Pos);
                                        ...
                                        CCB_ECDSASignTypeDef *pSignature)
{
  ...
  /* Set the mode and deactivate the interrupts */
  MODIFY_REG(PKA->CR, PKA_CR_MODE | PKA_CR_PROCENDIE | PKA_CR_RAMERRIE | PKA_CR_ADDRERRIE | PKA_CR_OPERRIE,
             HAL_CCB_PKA_MODE_ECDSA_VERIFICATION << PKA_CR_MODE_Pos);
...
static uint32_t PKA_ECDSAVerif_IsValidSignature(void)
{
  return (PKA->RAM[PKA_ECDSA_VERIF_OUT_RESULT] == 0xD60DUL) ? 1UL : 0UL;
}
...
/**
  ...
  * @param  pWrappingKey pointer to the Wrapping Key structure.
  * @param  pWrappedPrivateKeyBlob pointer to the related wrapped Private Key Blob.
  * @param  CCB_Operation pointer to CCB Operations
  * @retval HAL status.
  */
static HAL_StatusTypeDef CCB_ECC_ScalarMulBlobCreation(CCB_HandleTypeDef *hccb,
                                                       CCB_ECCMulCurveParamTypeDef *pCurveParam,
                                                       ...
                                                       uint8_t CCB_Operation)
{
  ...
  if (hccb->State == HAL_CCB_STATE_READY)
  {
    ...
    if (CCB_Operation == HAL_CCB_ECC_SCALAR_MUL_CPU_BLOB_CREATION)
    {
    ...
    else if (CCB_Operation == HAL_CCB_ECC_SCALAR_MUL_RNG_BLOB_CREATION)
    {
    ...
    for (offset = 0; offset < cipherkey_size; offset++)
    {
      ...
      if ((offset % 4UL) == 3UL)
      {
        ...
        for (count = 0U; count < CCB_BLOCK_SIZE; count++)
        {
          pWrappedPrivateKeyBlob->pWrappedKey[cipherkey_size - (countBlock + count + 1U)]
            = READ_REG(SAES->DOUTR);
        }
        countBlock += 4UL;
      }
                                                  ...
                                                  uint8_t VerifOperation)
{
  ...
  /* Set Operation in CCB */
  MODIFY_REG(CCB->CR, CCB_CR_CCOP, HAL_CCB_ECC_SCALAR_MUL_BLOB_USE);
  ...
  /* Initialize PKA */
  if (Protect_PKA_Init(hccb, HAL_CCB_ECC_SCALAR_MUL_BLOB_USE) != HAL_OK)
  {
    /* Set state and return error */
    hccb->State = HAL_CCB_STATE_ERROR;
    return HAL_ERROR;
  ...
  /* Wrapping Key configuration */
  if (WrappingKeyConfiguration(hccb, HAL_CCB_ECC_SCALAR_MUL_BLOB_USE, pWrappingKey) != HAL_OK)
  {
    /* return error */
    return HAL_ERROR;
  }
  ...
  for (offset = 0UL; offset < cipherkey_size; offset++)
  {
    ...
    if ((offset % 4UL) == 0x3UL)
    {
      ...
      {
        PKA->RAM[PKA_ECDSA_SIGN_IN_K + (countBlock + count_ram)] = CCB_MAGIC_VALUE;
      }
      countBlock += 4UL;
    }
  }
  ...
  {
    /* Final phase processing */
    if (CCB_BlobUse_FinalPhase(hccb, HAL_CCB_ECC_SCALAR_MUL_BLOB_USE, pCurveParam->modulusSizeByte) != HAL_OK)
    {
      /* return error */
      return HAL_ERROR;
    }
  ...
  /* Check PKA Operation error result */
  if ((PKA->RAM[PKA_ECC_SCALAR_MUL_OUT_ERROR]) !=  HAL_CCB_PKA_ERROR_OPERATION_NONE)
  {
    /* Set state and return error */
    hccb->State = HAL_CCB_STATE_ERROR;
    return HAL_ERROR;
  ...
  else /* (VerifOperation == CCB_VERIF_OPERATION_ENABLED) */
  {
    ...
    RandomNbr = (uint8_t)(RNG->DR & 0xFFUL);
    count = 0;
    while (count < RandomNbr)
    {
      count++;
                                                 ...
                                                 CCB_RSAKeyBlobTypeDef *pWrappedPrivateKeyBlob)
{
  ...
  /* Set Operation in CCB */
  MODIFY_REG(CCB->CR, CCB_CR_CCOP, HAL_CCB_MODULAR_EXP_CPU_BLOB_CREATION);
  ...
  /* Initialize PKA */
  if (Protect_PKA_Init(hccb, HAL_CCB_MODULAR_EXP_CPU_BLOB_CREATION) != HAL_OK)
  {
    /* Set state and return error */
    hccb->State = HAL_CCB_STATE_ERROR;
    return HAL_ERROR;
  ...
  /* Wrapping Key configuration */
  if (WrappingKeyConfiguration(hccb, HAL_CCB_MODULAR_EXP_CPU_BLOB_CREATION, pWrappingKey) != HAL_OK)
  {
    /* return error */
    return HAL_ERROR;
  }
  ...
  for (offset = 0UL; offset < cipherkey_size; offset++)
  {
    ...
    if ((offset % 4UL) == 3UL)
    {
      ...
      for (count = 0U; count < CCB_BLOCK_SIZE; count++)
      {
        pWrappedPrivateKeyBlob->pWrappedExp[cipherkey_size - (countBlock + count + 1UL)]
          = READ_REG(SAES->DOUTR);
      }
      countBlock += 4UL;
    }
  ...
  for (offset = 0; offset < cipherkey_size; offset++)
  {
    ...
    if ((offset % 4UL) == 3UL)
    {
      ...
      for (count = 0U; count < CCB_BLOCK_SIZE; count++)
      {
        pWrappedPrivateKeyBlob->pWrappedPhi[cipherkey_size - (countBlock + count + 1UL)]
          = READ_REG(SAES->DOUTR);
      }
      countBlock += 4UL;
    }
  ...
  /* Final phase processing */
  if (CCB_BlobCreation_FinalPhase(hccb, HAL_CCB_MODULAR_EXP_CPU_BLOB_CREATION, pWrappedPrivateKeyBlob->pTag,
                                                   ...
                                                   uint8_t VerifOperation)
{
  ...
  /* Set Operation in CCB */
  MODIFY_REG(CCB->CR, CCB_CR_CCOP, HAL_CCB_MODULAR_EXP_BLOB_USE);
  ...
  /* Initialize PKA */
  if (Protect_PKA_Init(hccb, HAL_CCB_MODULAR_EXP_BLOB_USE) != HAL_OK)
  {
    /* Set state and return error */
    hccb->State = HAL_CCB_STATE_ERROR;
    return HAL_ERROR;
  ...
  /* Wrapping Key configuration */
  if (WrappingKeyConfiguration(hccb, HAL_CCB_MODULAR_EXP_BLOB_USE, pWrappingKey) != HAL_OK)
  {
    /* return error */
    return HAL_ERROR;
  }
  ...
  for (offset = 0UL; offset < cipherkey_size; offset++)
  {
    WRITE_REG(SAES->DINR, pWrappedPrivateKeyBlob->pWrappedExp[cipherkey_size - (offset + 1U)]);
    ...
    if ((offset % 4UL) == 0x3UL)
    {
      ...
      for (count_ram = 0U; count_ram < 4U; count_ram++)
      {
        PKA->RAM[PKA_MODULAR_EXP_PROTECT_IN_EXPONENT + (countBlock + count_ram)] = CCB_MAGIC_VALUE;
      }
      countBlock += 4UL;
    }
  }
  ...
  for (offset = 0UL; offset < cipherkey_size; offset++)
  {
    WRITE_REG(SAES->DINR, pWrappedPrivateKeyBlob->pWrappedPhi[cipherkey_size - (offset + 1UL)]);
    ...
    if ((offset % 4UL) == 0x3UL)
    {
      ...
      for (count_ram = 0U; count_ram < 4U; count_ram++)
      {
        PKA->RAM[PKA_MODULAR_EXP_PROTECT_IN_PHI + (countBlock + count_ram)] = CCB_MAGIC_VALUE;
      }
      countBlock += 4UL;
    }
  }
  ...
  /* Final phase processing */
  if (CCB_BlobUse_FinalPhase(hccb, HAL_CCB_MODULAR_EXP_BLOB_USE, pParam->modulusSizeByte) != HAL_OK)
  {
    /* return error */
    return HAL_ERROR;
  }
  ...
  /* Check PKA Operation error result */
  if ((PKA->RAM[PKA_MODULAR_EXP_OUT_ERROR]) !=  HAL_CCB_PKA_ERROR_OPERATION_NONE)
  {
    /* Set state and return error */
    hccb->State = HAL_CCB_STATE_ERROR;
    return HAL_ERROR;
  ...
  else /* CCB_VERIF_OPERATION_ENABLED */
  {
    ...
    {
      /* Check out point */
      if (PKA->RAM[PKA_MODULAR_EXP_OUT_RESULT + offset] != pReferenceModularExp[offset])
      {
        return HAL_ERROR;
      }
...
{
  uint32_t index;
  __IO uint8_t randomNbr;

  PKA->RAM[PKA_RAM_SIZE - 1U] = CCB_MAGIC_VALUE;
  ...
  if (PKA->RAM[PKA_RAM_SIZE - 1U] == CCB_MAGIC_VALUE)
  {
    if (CCB_RNG_Wait_SET_FLAG(RNG_SR_DRDY, HAL_CCB_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
    {
      return HAL_ERROR;
